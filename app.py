...
# -------------------------------------------------------------
# 파일 폴더 생성 및 컴포넌트 선언 (캐시 방지 v14)
# -------------------------------------------------------------
component_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "snake_v14")
os.makedirs(component_dir, exist_ok=True)
with open(os.path.join(component_dir, "index.html"), "w", encoding="utf-8") as f:
    f.write(GAME_HTML)

snake_game = components.declare_component("snake_v14", path=component_dir)
...
st.info("⬅️⬆️➡️⬇️ 방향키 조작! ⏳**15초 내에 밥을 안 먹거나 벽에 부딪히면 몸통이 줄어드니** 서두르세요!")
...
with st.expander("🕹️ 1. 게임 기본 설명"):
    st.markdown("""
    * **조작 방법**: 키보드 방향키 (⬅️ ⬆️ ➡️ ⬇️)
    * **게임 시작/부활**: 닉네임 입력 후 `[게임 시작]` 버튼 클릭 또는 `[Space Bar]` 입력
    * **핵심 룰**: 맵에 등장하는 기본 먹이(빨간 네모)를 먹으면 **+10점**을 획득하며 꼬리가 길어집니다.
    * **난이도 상승 (Speed Rush!)**: 
      * 점수가 **50점** 오를 때마다 지렁이의 **이동 속도가 점점 빨라집니다.**
      * 점수가 **100점** 오를 때마다 맵에 등장하는 기본 먹이 개수가 증가합니다. (최대 5개)
    * **목숨(하트) 시스템**: 총 **3개(❤️❤️❤️)**의 목숨이 주어집니다.
      * 벽이나 자기 몸통에 부딪히면 목숨이 1개 깎이며, **점수 감점 없이 몸통만 3칸 줄어든 채** 중앙에서 부활합니다. 
      * 하트 3개를 모두 잃으면 게임이 종료됩니다.
    """)

with st.expander("🎁 2. 미스터리 아이템 도감 (❓)"):
    st.markdown("""
    맵 곳곳에 랜덤으로 등장하는 **물음표(❓) 상자** 안에는 아래 9가지 아이템 중 하나가 숨겨져 있습니다. 어떤 효과가 나올지 모르는 쫄깃함을 즐겨보세요!

    | 맵 등장 | 숨겨진 아이템 | 효과 설명 |
    | :---: | :---: | :--- |
    | **❓** | 🍎 **보너스 사과** | 럭키! 점수 **+50점** 즉시 획득 |
    | **❓** | 🍓 **슈퍼 보너스 딸기** | 대박! 점수 **+100점** 즉시 획득 |
    | **❓** | 🍌 **느릿느릿 바나나** | 5초간 지렁이 이동 속도 **대폭 감소** (쉬어가는 타임) |
    | **❓** | 🍇 **초스피드 포도** | 5초간 지렁이 이동 속도 **급상승** (벽 충돌 주의!) |
    | **❓** | ☁️ **암흑 구름** | 2초간 눈앞이 캄캄해짐 (**화면 가려짐**) |
    | **❓** | 🌀 **반전 터널** | 머리와 꼬리 위치가 뒤바뀌며 **이동 방향이 즉시 반전됨** |
    | **❓** | 🍄 **독버섯** | 5초간 **방향키 조작이 반대로** 바뀜 (좌↔우, 상↔하) |
    | **❓** | 🐛 **매직 애벌레** | 5초간 50% 확률로 **왕꿈틀이(거대화)** 또는 **꼬마(축소)** 변신 |
    | **❓** | 🍊 **폭탄 오렌지** | 앗, 꽝! 점수 **-30점** 감점 |
    """)

with st.expander("⚠️ 3. 플레이 주의사항 및 꿀팁 (Tip!)"):
    st.markdown("""
    1. **⏳ 15초 굶주림(아사) 타이머 주의!**
       * 상단에 **15초 카운트다운 타이머**가 돌아갑니다.
       * 15초 안에 아무 먹이(또는 아이템)도 먹지 못하면 **"⚠️ 아사 위기!"** 경고와 함께 **몸통이 2칸 깎여 나갑니다.** 
       * 먹이를 먹을 때마다 타이머는 다시 15초로 꽉 채워지니 부지런히 움직이세요!

    2. **🍄 독버섯을 먹었을 땐 머리 색을 보세요!**
       * 방향키가 반대로 바뀌는 독버섯에 걸리면 지렁이의 머리가 **빨간색(경고색)**으로 변합니다. 당황하지 말고 침착하게 반대로 조작하세요!

    3. **💥 전략적 충돌 (몸통 다이어트)**
       * 몸집이 너무 커져서 갇힐 위기라면? 충돌해서 죽어도 **점수는 깎이지 않습니다!**
       * 하트에 여유가 있다면 과감하게 벽에 부딪혀 목숨 1개를 내주고 **몸통을 3칸 줄이는 다이어트**를 하는 것도 고득점을 향한 훌륭한 전략입니다.

    4. **👑 명예의 전당 (1위의 영광)**
       * 우측 랭킹보드에서 **1위(🥇)**를 차지한 플레이어의 닉네임 옆에는 **달성 날짜와 시간**이 영원히 박제됩니다. 실시간 1위에 도전하세요!
    """)
